...
If a plan exists, read the **Gauntlette Review Report** table at the bottom of the plan. Find the last skill with status `DONE` or `SKIPPED` (any SKIPPED variant counts as completed) — the *next* skill in the pipeline is the current stage. If no skill has run or been skipped, the current stage is `/survey-and-plan`.

If no plan exists, the current stage is `/survey-and-plan`.
...
1. /survey-and-plan — Create the plan document, orient on codebase
...
```
No active plan. Run /survey-and-plan to start.
```
